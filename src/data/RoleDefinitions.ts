  ...
  COMPOSITIONS: [
    {
      ...
      composition: { murderers: 1, villagers: 0, detectives: 0, accomplices: 0 },
      description: "テスト用：殺人者のみ"
    },
    {
      playerRange: { min: 2, max: 2 },
      composition: { murderers: 1, villagers: 1, detectives: 0, accomplices: 0 },
      description: "殺人者1人、村人1人"
    },
    {
      playerRange: { min: 3, max: 3 },
      composition: { murderers: 1, villagers: 1, detectives: 1, accomplices: 0 },
    ...
    },
    {
      ...
      composition: { murderers: 1, villagers: -1, detectives: 1, accomplices: 1 },
      description: "標準構成：殺人者1人、探偵1人、共犯者1人、残り村人"
    },
    {
      playerRange: { min: 11, max: 16 },
      composition: { murderers: 1, villagers: -1, detectives: 2, accomplices: 1 },
      description: "大人数対応：殺人者1人、探偵2人、共犯者1人、残り村人"
    },
    {
      playerRange: { min: 17, max: 20 },
      composition: { murderers: 2, villagers: -1, detectives: 2, accomplices: 1 },
...
export function getRoleComposition(playerCount: number): RoleComposition {
  if (playerCount < ROLE_COMPOSITION_RULES.MIN_PLAYERS) {
    throw new Error(`最低${ROLE_COMPOSITION_RULES.MIN_PLAYERS}人のプレイヤーが必要です`);
  }
  
  if (playerCount > ROLE_COMPOSITION_RULES.MAX_PLAYERS) {
    throw new Error(`最大${ROLE_COMPOSITION_RULES.MAX_PLAYERS}人まで対応しています`);
  }
  
  // 該当する構成を見つける
  const rule = ROLE_COMPOSITION_RULES.COMPOSITIONS.find(
    rule => playerCount >= rule.playerRange.min && playerCount <= rule.playerRange.max
  );
  ...
  const composition: RoleComposition = {
    ...
    villagers: rule.composition.villagers === -1 
      ? playerCount - rule.composition.murderers - rule.composition.detectives - rule.composition.accomplices
...
export function validateRoleComposition(composition: RoleComposition, playerCount: number): boolean {
  const total = composition.murderers + composition.villagers + composition.detectives + composition.accomplices;
