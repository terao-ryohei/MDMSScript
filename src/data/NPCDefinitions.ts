...
export const NPC_SPAWN_SETTINGS = {
  ...
  // エリア選択方式
  AREA_SELECTION_MODE: "weighted_random" as "random" | "weighted_random" | "preset_order"
};
 ...
 */
export function getRandomSpawnLocationForNPC(npc: NPCDefinition): NPCSpawnLocation | null {
  ...
  const areaLocations = NPC_SPAWN_LOCATIONS.filter(location => 
...
export function getAreaInfo(areaName: string) {
  const area = Object.values(SPAWN_AREAS).find(area => area.name === areaName);
  return area || null;
}
 ...
 * プレイヤーから十分離れた場所かチェック
 */
export function isValidSpawnDistance(location: Vector3, playerLocations: Vector3[]): boolean {
  const minDistance = NPC_SPAWN_SETTINGS.MIN_DISTANCE_FROM_PLAYERS;
  
  return playerLocations.every(playerLoc => {
    const distance = Math.sqrt(
      Math.pow(location.x - playerLoc.x, 2) +
      Math.pow(location.y - playerLoc.y, 2) +
      Math.pow(location.z - playerLoc.z, 2)
    );
