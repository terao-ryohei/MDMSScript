...
export const OBJECTIVE_DEFINITIONS: Record<string, ObjectiveDefinition> = {
  ...
  hide_adultery: {
    ...
    conditions: [
      { type: "keep_secret", target: "royal_affair", description: "王室の秘密を守る" },
  ...
  find_criminal_roles: {
    ...
    conditions: [
      ...
      { type: "gather_evidence", value: 3, description: "3つ以上の証拠を収集する" }
    ],
    hints: [
      "怪しい行動を監視しよう",
      "他プレイヤーの発言に注意を払おう",
  ...
  assassinate_cariostro: {
    ...
    conditions: [
      { type: "eliminate_target", target: "cariostro", description: "カリオストロを殺害する" },
      { type: "conceal_identity", description: "ホムンクルスとしての正体を隠し通す" },
  ...
  create_philosophers_stone: {
    ...
    conditions: [
      { type: "gather_rare_materials", value: 5, description: "5つの希少材料を集める" },
      { type: "complete_transmutation", description: "最高レベルの錬成を成功させる" },
  ...
  infiltrate_ladys_chamber: {
    ...
    conditions: [
      { type: "infiltrate_chamber", target: "lady_room", description: "領主の妻の部屋に忍び込む" },
      { type: "wear_dress", description: "貴婦人のドレスを着用する" },
      { type: "appear_in_hall", target: "great_hall", description: "大広間に堂々と現れる" },
  ...
  acquire_lords_seal: {
    ...
    conditions: [
      ...
      { type: "obtain_seal", target: "lords_golden_seal", description: "金印を所持する" }
    ],
    hints: [
      "複数の入手ルートが存在する",
      "好感度を最大まで上げるのが最も確実",
  ...
  promotion_to_knight_guard: {
    ...
    conditions: [
      { type: "complete_patrols", value: 5, description: "巡回任務を5回完了する" },
      { type: "equipment_maintenance", value: 3, description: "武具点検を3回完了する" },
  ...
  create_immortality_elixir: {
    ...
    conditions: [
      { type: "gather_forbidden_materials", value: 7, description: "禁断の材料を7つ集める" },
      { type: "infiltrate_court_lab", description: "宮廷錬金術師の執務室に潜入する" },
  ...
  create_golden_apple: {
    ...
    points: { completion: 150, bonus: 100 },
    conditions: [
      { type: "gather_special_plants", value: 3, description: "特殊な植物を3種類採集する" },
      { type: "create_night_vision", description: "暗視ポーションを作成する" },
      { type: "craft_golden_apple", description: "金のリンゴを調合する" },
      { type: "deliver_item", target: "golden_apple", description: "金のリンゴを納品する" }
    ],
    hints: [
      "パーゴラには希少な植物が育っている",
  ...
  earn_tuition: {
    ...
    conditions: [
      ...
      { type: "work_part_time", value: 5, description: "アルバイトを5回こなす" },
  ...
  improve_guild_rank: {
    ...
    conditions: [
      ...
      { type: "complete_quests", value: 5, description: "ギルドクエストを5回完了する" },
      { type: "gain_reputation", value: 1000, description: "1000の評判ポイントを獲得する" }
    ],
    hints: [
      "近衛兵との良好な関係は重要な情報源だ",
      "領主への敬意は社会的地位を示す",
  ...
  complete_research: {
    ...
    points: { completion: 140, bonus: 90 },
    conditions: [
      { type: "collect_materials", value: 5, description: "必要な材料を5つ集める" },
  ...
  earn_profit: {
    ...
    conditions: [
      { type: "make_deals", value: 5, description: "5回以上の取引を成功させる" },
  ...
  guild_reputation: {
    ...
    conditions: [
      ...
      { type: "recruit_members", value: 2, description: "新メンバーを2人勧誘する" }
    ],
    hints: [
      "他プレイヤーにギルドの魅力を伝えよう",
      "依頼の成功率を上げよう"
  ...
  legendary_weapon: {
    ...
    conditions: [
      { type: "gather_rare_materials", value: 3, description: "希少材料を3つ入手する" },
  ...
  tavern_prosperity: {
    ...
    conditions: [
      { type: "host_meetings", value: 3, description: "3回の秘密会議を開催する" },
      { type: "collect_information", value: 5, description: "5つの重要情報を収集する" }
    ],
    hints: [
      "お客さんの会話に耳を傾けよう",
      "美味しい料理で常連を作ろう"
  ...
  social_butterfly: {
    ...
    conditions: [
      { type: "befriend_players", value: 5, description: "5人以上と友好関係を築く" }
    ],
    hints: [
      "積極的に話しかけよう",
      "他人の話をよく聞こう"
  ...
  information_broker: {
    ...
    points: { completion: 120, bonus: 80 },
    conditions: [
      { type: "collect_secrets", value: 7, description: "7つの秘密情報を収集する" }
    ],
    hints: [
      "会話の中の矛盾を見つけよう",
      "情報の価値を見極めよう"
...
export const OBJECTIVES_BY_CATEGORY: Record<ObjectiveCategory, string[]> = {
  [ObjectiveCategory.ROLE_PRIMARY]: [
    "identify_murderer", "avoid_execution", "support_murderer"
  ],
  [ObjectiveCategory.JOB_TASK]: [
    "hide_adultery", "judge_criminal", "find_criminal_roles", "assassinate_cariostro", "create_philosophers_stone", "infiltrate_ladys_chamber", "acquire_lords_seal", "promotion_to_knight_guard", "create_immortality_elixir", "create_golden_apple", "earn_tuition", "maintain_order", "complete_research", "earn_profit",
    "guild_reputation", "legendary_weapon", "tavern_prosperity",
    "perfect_garden", "perfect_service", "surpass_master"
  ],
  [ObjectiveCategory.PERSONAL]: [
    "social_butterfly", "information_broker", "survivor"
  ],
...
export const OBJECTIVES_BY_DIFFICULTY: Record<ObjectiveDifficulty, string[]> = {
  [ObjectiveDifficulty.EASY]: [
    "perfect_garden", "social_butterfly", "earn_tuition"
  ],
  [ObjectiveDifficulty.NORMAL]: [
    "identify_murderer", "maintain_order", "promotion_to_knight_guard", "earn_profit", 
    "guild_reputation", "tavern_prosperity", "perfect_service",
    "information_broker", "improve_guild_rank"
  ],
  [ObjectiveDifficulty.HARD]: [
    "avoid_execution", "support_murderer", "complete_research",
    "judge_criminal", "find_criminal_roles", "infiltrate_ladys_chamber", "create_golden_apple", "survivor"
  ],
  [ObjectiveDifficulty.EXPERT]: [
    "hide_adultery", "assassinate_cariostro", "create_philosophers_stone", "acquire_lords_seal", "create_immortality_elixir", "legendary_weapon", "surpass_master"
  ]
};
...
export function getRandomPersonalObjective(): string {
  ...
  return personalObjectives[Math.floor(Math.random() * personalObjectives.length)];
}

/**
 * 難易度別ランダム目標を取得
 */
export function getRandomObjectiveByDifficulty(difficulty: ObjectiveDifficulty): string {
