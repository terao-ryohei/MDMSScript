...
export const createPlayerSelectionUI = (
	...
	if (includeAll && players.length > 1) {
		form.button("全プレイヤー", "textures/ui/permissions_op_crown");
	}
...
export const createAdminMenuUI = (title: string): ActionFormData => {
	return new ActionFormData()
		.title(title)
		.button("ゲーム管理", "textures/ui/gear")
		.button("プレイヤー管理", "textures/ui/permissions_member_star")
		.button("設定変更", "textures/ui/settings_glyph_color_2x")
		.button("統計情報", "textures/ui/book_metatag_default")
...
export const createPaginatedMenu = (
	...
	// ページネーションボタン
	if (currentPage > 0) {
		form.button("◀ 前のページ", "textures/ui/arrow_left");
	}
	if (currentPage < totalPages - 1) {
		form.button("次のページ ▶", "textures/ui/arrow_right");
	}
...
export const createStatusDisplayUI = (
	...
	const body = statusItems
		.map((item) => {
			const color = item.isGood !== false ? "§2" : "§c";
...
export const createDetailViewUI = (
	...
	form.body(body);
	form.button("戻る", "textures/ui/arrow_left");
