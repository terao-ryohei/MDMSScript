 ...
 * 基本的なActionFormを作成
 */
export const createActionForm = (title: string, body?: string): ActionFormData => {
 ...
 * 基本的なMessageFormを作成
 */
export const createMessageForm = (title: string, body: string): MessageFormData => {
 ...
 * エラー表示ダイアログ
 */
export const showErrorDialog = async (player: Player, title: string, message: string): Promise<void> => {
  try {
    const form = createMessageForm(title, `§c${message}`)
      .button1("確認");
 ...
 * ステータス表示用のテキスト作成
 */
export const createStatusText = (label: string, value: string, good: boolean = true): string => {
