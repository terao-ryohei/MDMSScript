...
export async function showAbilitySelection(player: Player): Promise<void> {
	try {
		const playerAbilities = getPlayerAbilities(player.id);
		const availableAbilities = Array.from(playerAbilities.entries()).filter(
			([_, state]) => state.status === AbilityStatus.AVAILABLE,
		);
		...
		for (const [abilityId, state] of availableAbilities) {
			...
			if (definition) {
				...
				form.button(
					`${statusIcon} §f${definition.name} ${usesText}`,
					"textures/ui/gear",
				);
			}
		}
...
async function showAbilityConfirmation(
	...
	try {
		...
		const playerState = getPlayerAbilities(player.id);
		...
		if (definition.requiresTarget) {
			// 対象選択が必要な場合
			await showTargetSelection(player, abilityId, definition);
			return;
		} else {
			confirmationText += "\n§7この能力を使用しますか？";
		}
		...
		// 能力使用実行
		const result = await useAbility(player, abilityId);
...
async function showTargetSelection(
	...
	try {
		if (definition.targetType === AbilityTargetType.PLAYER) {
			const alivePlayers = world
				.getAllPlayers()
				.filter((p) => isPlayerAlive(p) && p.id !== player.id);
			...
			for (const target of alivePlayers) {
				...
				form.button(
					`${statusIcon} §f${target.name} ${distanceText}`,
...
async function showTargetConfirmation(
	...
	try {
		...
		// 能力使用実行
		const result = await useAbility(player, abilityId, target.id);
 ...
 * 能力状態アイコン取得
 */
function getAbilityStatusIcon(state: AbilityInstanceState): string {
	switch (state.status) {
 ...
 * 能力状態テキスト取得
 */
function getAbilityStatusText(state: AbilityInstanceState): string {
	switch (state.status) {
		case AbilityStatus.AVAILABLE:
			return "§a使用可能";
		case AbilityStatus.COOLDOWN:
