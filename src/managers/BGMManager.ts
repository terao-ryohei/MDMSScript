...
let playbackTimers: Map<string, number> = new Map();
	...
	stopBGM();
		...
		system.runTimeout(() => {
			if (track.melodies && track.melodies.length > 0) {
				track.melodies.forEach((melody: Melody) =>
					playMelody(melody, bgm),
				);
			}
		}, 5);
...
function playNote(instrument: Instrument, note: Note, volume: number): void {
		...
		const effectiveVolume = Math.min(
			...
			Math.max(
				0.0,
				(note.volume || 1.0) * volume * audioSettings.masterVolume,
			),
		);
